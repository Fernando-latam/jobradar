...
def main():
    ...
    if not jobs:
        ...
        report.build([], [], {"bruto": 0, "apos_dedup": 0, "match_titulo": 0,
                              "elegiveis": 0, "talvez": 0, "bloqueadas": 0},
                     sources.DIAGNOSTICS)
        return 1

    open_jobs, maybe_jobs, stats = filters.run(jobs)

    print("\n" + "-" * 60)
    print(f"  Coletadas .......... {stats['bruto']}")
    print(f"  Após deduplicar .... {stats['apos_dedup']}")
    print(f"  Da sua área ........ {stats['match_titulo']}")
    print(f"  ELEGÍVEIS .......... {stats['elegiveis']}")
    print(f"  Talvez ............. {stats['talvez']}")
    print(f"  Bloqueadas ......... {stats['bloqueadas']}")
    print("-" * 60)

    path = report.build(open_jobs, maybe_jobs, stats, sources.DIAGNOSTICS)
